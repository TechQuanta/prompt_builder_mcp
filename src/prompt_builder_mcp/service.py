...
def get_prompt_schema() -> dict[str, Any]:
    properties = {"schema_version": {"type": "string", "const": SCHEMA_VERSION, "description": "Optional schema version marker; if sent, it must equal 1.3."}, "user_prompt": {"type": "string", "minLength": 1, "maxLength": 8000, "pattern": r"\S", "description": "The raw request. It is trimmed before validation and fenced as data when rendered."}, "prompt_nature": {"type": "integer", "minimum": 0, "maximum": 100, "default": 50, "description": "Creative latitude from precise/literal (0) to highly exploratory (100); the default 50 is omitted from output."}, "constraints": {"type": "array", "maxItems": 10, "uniqueItems": True, "items": {"type": "string", "minLength": 1, "maxLength": 200, "pattern": r"\S"}, "description": "Free-text rules; blank entries are dropped and duplicates removed during normalization."}}
    ...
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "$id": "https://techquanta.dev/schemas/prompt-brief-1.3.json", "title": "Prompt Builder Brief", "description": "Input contract for validate_brief and build_prompt_variants.", "type": "object", "required": ["user_prompt"], "additionalProperties": False, "properties": properties, "dependentRequired": {field: ["task"] for field in all_task_fields}, "allOf": audience_rules + task_rules + framework_rules, "x-task-fields": {key: list(value) for key, value in TASK_FIELDS.items()}, "x-audience-by-task": {key: list(value) for key, value in AUDIENCE_BY_TASK.items()}, "x-framework-languages": {key: list(value) for key, value in FRAMEWORK_LANGUAGES.items()}, "x-normalization": ["trim user_prompt", "trim each constraint, drop blanks, drop duplicates", "drop empty constraints array", "drop prompt_nature when equal to default", "never coerce types"], "x-error-contract": {"shape": {"valid": "bool", "score": "0-100", "errors": "list", "warnings": "list", "missing_fields": "list", "suggested_fields": "list"}, "rules": ["Never raise for bad input", "Report all errors", "Invalid briefs score 0"]}, "x-completeness": {"user_prompt_present": 30, "user_prompt_specific_8plus_words": 10, "task_selected": 20, "task_fields_filled_proportional": 25, "tone": 5, "output": 5, "detail": 5}, "x-warnings": {"vague_prompt": "user_prompt has fewer than 8 words", "control_may_not_apply": "tone/output/detail on an Image prompt", "output_conflicts_with_form": "Write with JSON/Table/Timeline output", "detail_length_tension": "Short length with In-depth detail", "blank_constraints_dropped": "normalization removed blank constraints", "duplicate_constraints_removed": "normalization removed duplicate constraints"}, "x-rendering": {"fence_user_prompt": "Wrap user_prompt in <user_request>...</user_request> and escape literal tags", "no_raw_field_dump_for_prompt_nature": "Render creative latitude as natural language", "task_aware_closers": "Each variant closer depends on task", "variants_must_differ": "focused is compact; detailed and structured use a preamble and bullets"}}
